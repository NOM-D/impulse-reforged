...
AddCSLuaFile("core/util/sh_logs.lua")
AddCSLuaFile("core/util/cl_util.lua")
AddCSLuaFile("core/util/sh_util.lua")
AddCSLuaFile("core/util/sv_util.lua")
...
if engine.ActiveGamemode() == "impulse-reforged" then
    ...
    for k, v in pairs(engine.GetGamemodes()) do
        if v.name:find("impulse") and v.name ~= "impulse-reforged" then gs = gs .. v.name .. "\n" end
    end

    SetGlobalString("impulse_fatalerror",
        "No schema loaded. Please place the schema in your gamemodes folder, then set it as your gamemode.\n\nInstalled available schemas:\n" ..
        gs)
end
...
local function IncludeResourcesFromDir(dir)
    local total = 0
    ...
    for k, v in pairs(folders) do
        total = total + IncludeResourcesFromDir(dir .. v .. "/")
    end
    return total
end
...
local function IncludeContent()
    ...
    local total = 0
    total = total + IncludeResourcesFromDir("gamemodes/impulse-reforged/content/")

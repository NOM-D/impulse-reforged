...
function PLAYER:SetTeam(teamID)
    ...
    if ( !teamData ) then
        impulse.Logs:Debug(ply:Nick() .. " tried to join invalid team " .. teamID)
        return false, "Invalid team ID!"
    end

    local teamPlayers = team.NumPlayers(teamID)
    if ( teamData.max and teamPlayers >= teamData.max ) then
        impulse.Logs:Debug(ply:Nick() .. " tried to join team " .. teamID .. " but it was full")
    ...
    impulse.Logs:Debug("TeamData is setting " .. self:Nick() .. " model to " .. (teamModel or selfTable.impulseDefaultModel))
    ...
    if ( teamData.bodygroups ) then
        ...
        for v, bodygroupData in pairs(teamData.bodygroups) do
            impulse.Logs:Debug("TeamData is setting bodygroup to "..bodygroupData[1].." with subGroup "..bodygroupData[2])
...
function PLAYER:SetTeamRank(rankID)
    ...
    if ( !teamData ) then self:Notify("Player does not have a valid team selected!") return false end
        ...
        for index, bodygroupData in pairs(rankData.bodygroupOverrides) do
            ...
            if isClassSpecific then
                ...
                for bodygroup, subBodygroup in pairs(bodygroupData) do
                    impulse.Logs:Debug("RankData is setting class-specific bodygroup to "..bodygroup.." with subGroup "..subBodygroup)

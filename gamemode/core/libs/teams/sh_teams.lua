...
function impulse.Teams:Register(teamData)
    ...
    local index = self.List[niceName] or table.Count(self.List) + 1

    teamData.index = index
    teamData.codeName = niceName

    team.SetUp(index, teamData.name, teamData.color, false)
    ...
    self.List[niceName] = index
    self.Stored[index] = teamData
...
function impulse.Teams:GetByIndex(teamIndex)
    return self.Stored[teamIndex]
end
...
function impulse.Teams:FindTeam(identifier)
    ...
    for teamIndex, teamData in ipairs(impulse.Teams.Stored) do
        if (impulse.Util:StringMatches(tostring(teamData.name), identifier)) then
            return teamData
        elseif (teamIndex == tonumber(identifier)) then
            return teamData
        end
    end
end
...
function PLAYER:CanBecomeTeam(teamID, notify)
    ...
    if (self:GetNetVar(NET_IS_ARRESTED, false)) then return false end
...
function PLAYER:GetTeamClass()
    return self:GetNetVar(NET_TEAM_CLASS, nil)
end

---Get the player's team rank
---@return number?
function PLAYER:GetTeamRank()
    return self:GetNetVar(NET_TEAM_RANK, nil)
end
...
function PLAYER:GetTeamClassName()
    ...
    local plyClass = self:GetNetVar(NET_TEAM_RANK, nil)
...
function PLAYER:GetTeamRankName()
    ...
    local plyRank = self:GetNetVar(NET_TEAM_RANK, nil)

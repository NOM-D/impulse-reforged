...
function PLAYER:SetHunger(amount)
    return self:SetNetVar(NET_HUNGER, math.Clamp(amount, 0, 100))
end
...
function PLAYER:FeedHunger(amount)
    return self:SetHunger(amount + self:GetNetVar(NET_HUNGER, 100))
end
...
function PLAYER:TakeHunger(amount)
    return self:SetHunger(self:GetNetVar(NET_HUNGER, 100) - amount)
end
...
function PANEL:Init()
    ...
    if LocalPlayer():GetNetVar(NET_GROUP_NAME, "") != "" then
        self:ShowGroup()
    else
        self:NoGroup()
    end
...
function PANEL:ShowGroup()
    ...
    lbl:SetText(LocalPlayer():GetNetVar(NET_GROUP_NAME, "Unknown Name"))
    ...
    lblM:SetText("")
    lblM:SetFont("Impulse-Elements18-Shadow")
    lblM:SetPos(5, 32)

    local lbl = vgui.Create("DLabel", sheet)
    lbl:SetText("Your rank: "..LocalPlayer():GetNetVar(NET_GROUP_RANK, "Unknown Rank"))
    ...
    function inv:DoClick()
        ...
        local gname = LocalPlayer():GetNetVar(NET_GROUP_NAME, nil)

        if not gname then return end

        for v, k in player.Iterator() do
            if k:GetNetVar(NET_GROUP_NAME, nil) then continue end
...
function PANEL:ShowInfo()
    local group = impulse.Group.Groups[1]
    local name = LocalPlayer():GetNetVar(NET_GROUP_NAME, "ERROR")
...
function PANEL:ShowAdmin()
    ...
    if group.Color then
        ...
        colPicker:SetColor(color_black)
    end

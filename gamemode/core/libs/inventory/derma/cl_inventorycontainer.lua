...
function PANEL:SetupContainer()
    ...
    self:SetTitle(hook.Run("GetContainerName", self.container) or
    (self.isLoot and "Storage Container" or "Loot Container"))
end
...
function PANEL:PaintOver(w, h)
    local capacity = self.container.GetCapacity and self.container:GetCapacity() or impulse.Config.InventoryMaxWeight
    ...
    if self.storageWeight and self.container then
        draw.SimpleText(self.storageWeight .. "kg/" .. capacity .. "kg", "Impulse-Elements18-Shadow", 355, 35, grey,
            TEXT_ALIGN_LEFT)
    end
end

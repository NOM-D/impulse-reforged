...
function impulse.Group:NetworkMetaDataToOnline(name)
    ...
    for v, k in player.Iterator() do
        local x = k:GetNetVar(NET_GROUP_NAME, nil)
...
function impulse.Group:NetworkRankToOnline(name, rankName)
    ...
    for v, k in player.Iterator() do
        local x = k:GetNetVar(NET_GROUP_NAME, nil)
        local r = k:GetNetVar(NET_GROUP_RANK, nil)
...
local function postCompute(self, name, rank, skipDb)
    ...
    self:SetNetVar(NET_GROUP_NAME, name)
    self:SetNetVar(NET_GROUP_RANK, rank)
...
function PLAYER:GroupRemove(name)
    ...
    self:SetNetVar(NET_GROUP_NAME, nil)
    self:SetNetVar(NET_GROUP_RANK, nil)
end

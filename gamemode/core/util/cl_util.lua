...
function impulse.Util:DynamicScaleFontSize(baseSize)
    ...
    local scaleFactor = Lerp(math.Clamp((screenHeight - baseHeight) / (targetHeight - baseHeight), 0, 1), baseScale,
        targetScale)
...
--- Adds a one-shot function to run when the player is ready
---@param callback fun()
function impulse.Util:AddPlayerReadyCallback(callback)
    local client = LocalPlayer()
    if (client && client._isReady) then
        callback()
        return
    else
        local hookTable = hook.GetTable()
        local initPostEntityHook = hookTable["InitPostEntity"]
        local hookId = "impulse.player.ready_" .. ((initPostEntityHook && table.Count(initPostEntityHook) + 1) || 1)
        hook.Add("InitPostEntity", hookId, function()
            LocalPlayer()._isReady = true
            callback()
            hook.Remove("InitPostEntity", hookId)
        end)
    end
end

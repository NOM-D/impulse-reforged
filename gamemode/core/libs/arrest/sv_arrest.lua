...
function PLAYER:Arrest()
    ...
    self:SetNetVar(NET_IS_INCOGNITO, true)
end

function PLAYER:UnArrest()
    self:SetNetVar(NET_IS_INCOGNITO, false)
...
function PLAYER:DragPlayer(ply)
    if ( self:CanArrest(ply) and ply:GetNetVar(NET_IS_INCOGNITO, false) ) then
...
function PLAYER:Jail(time, jailData)
    ...
    if ( !self:GetNetVar(NET_IS_INCOGNITO, false) ) then
        self:Arrest()
    end
